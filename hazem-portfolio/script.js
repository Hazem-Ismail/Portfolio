...
themeButton.addEventListener('click', async () => {
  ...
  const applyTheme = () => {
    ...
    try { localStorage.setItem('hazem-portfolio-v2-appearance', nextTheme); } catch (error) {}
    syncTheme();
  };
  // Respect the system preference while keeping the control available everywhere.
  ...
  if (!document.startViewTransition) {
    // A soft wave and color dissolve for browsers without snapshot transitions.
    ...
    root.classList.add('theme-dissolving');
    document.body.append(wave);
    applyTheme();
    try {
      await wave.animate([
        { transform: 'scale(0)', opacity: .6 },
        { transform: 'scale(.75)', opacity: .3, offset: .65 },
        { transform: 'scale(1)', opacity: 0 }
      ], { duration: 850, easing: 'cubic-bezier(.22,1,.36,1)' }).finished;
    } finally {
      wave.remove();
      root.classList.remove('theme-dissolving');
      themeChanging = false;
    }
    return;
  }
...
// Respect device accessibility settings and stop work in background tabs.
